...
总体说来, 这一次的项目还是做的很不错的. 从数据的抓取, 到数据的各种分析, 以及最后的分析结果的展示, 都基本上圆满地完成了. 特别是数据的各种分析, 基本上对有意义的统计分析都做到了覆盖, 除了有一些很难分析. 很难分析的就是每次提交的message. 项目的一开始就是要打算分析每次提交的具体信息的, 但最后放弃了. 由于每次提交的message都不是结构化的数据, 完全是靠提交者自定义的, 有的长, 有的短, 所以很难将其结构化. 同时每次提交的信息并没有手动或自动打上标签, 
